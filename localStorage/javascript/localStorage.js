...
WEBSTORAGE = function() {
    ...
    if (typeof localStorage !== "object") {
        alert("このブラウザはローカルストレージをサポートしておりません。");
        return;
    }
    ...
    return {
        ...
        getAll : function() {
            ...
            for (idx = 0, nLength = storage.length ; idx < nLength; idx++) {
                ...
                console.log("GETALL: " + idx + " key=" + storage.key(idx) + " value=" + storage.getItem(storage.key(idx)));
            }
            if (!nLength) {
        ...
        remove : function(key) {
            ...
            console.log("REMOVE: key=" + key + " value=" + storage.getItem(key));
            return ;
        },
        removeAll : function() {
            storage.clear();
